...
export const MissionLocationValidator: IAnswerValidator = {
  async validate(payload: AnswerPayload, step: IStep): Promise<ValidationResult> {
    ...
    if (!mission) {
      return { isCorrect: false, feedback: 'Invalid mission configuration' };
    }
    ...
    if (!submittedLocation) {
      return { isCorrect: false, feedback: 'Location data missing' };
    }

    if (!targetLocation) {
      return { isCorrect: false, feedback: 'Invalid mission configuration - no target location' };
    }
    ...
    if (!Number.isFinite(targetLocation.lat) || !Number.isFinite(targetLocation.lng)) {
      return { isCorrect: false, feedback: 'Invalid mission configuration - invalid target coordinates' };
    }
    ...
    if (isCorrect) {
      return { isCorrect: true, feedback: 'You found it!' };
    }

    return { isCorrect: false, feedback: `You're ${Math.round(distance)}m away. Keep searching!` };
  },
};
